...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create quest-log-3e7b
  fly apps create quest-log-3e7b-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app quest-log-3e7b
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app quest-log-3e7b-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'quest-log-3e7b-staging' does not match app name in config file 'quest-log-3e7b'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name quest-log-3e7b-db
  fly postgres attach --app quest-log-3e7b quest-log-3e7b-db

  fly postgres create --name quest-log-3e7b-staging-db
  fly postgres attach --app quest-log-3e7b-staging quest-log-3e7b-staging-db
  ```
